...
const ComparisonWizardForm = ({ setIsOpen, onDataChange }) => {
    ...
    const fetchDataByDoi = async value => {
        ...
        await sendDoiData(entryParsed, 'https://informed-manually-labrador.ngrok-free.app/api/get_meta_data_by_doi');
    };

    const fetchDataByAbstract = async value => {
        setIsLoadingCite(true);
        const entryParsed = value.trim();
        await sendAbstractData(entryParsed, 'https://informed-manually-labrador.ngrok-free.app/api/get_meta_data_by_abstract');
    };
    ...
    const checkPDFURL = async value => {
        if (value !== '') {
            setIsLoadingCite(true);
            const entryParsed = value.trim();
            await sendPDFURL(entryParsed, 'https://informed-manually-labrador.ngrok-free.app/api/check_pdf_url');
        }
    };
    ...
    const handleParse = async value => {
        ...
        try {
            ...
            setResults(_results);
            console.log(_results);
            await sendTitlePDFURL(_results[0].title, 'https://informed-manually-labrador.ngrok-free.app/api/get_pdf_by_title');
        } catch (e) {
    ...
    const handleAutocompleteSelect = async selected => {
        ...
        await sendTitlePDFURL(selected.label, 'https://informed-manually-labrador.ngrok-free.app/api/get_pdf_by_title');
    };
    ...
    const handleFormSubmit = async e => {
        ...
        const response = await sendComparisonDataToBackend(formData, 'https://informed-manually-labrador.ngrok-free.app/api/get_research_problems');
